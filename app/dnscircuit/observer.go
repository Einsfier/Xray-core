...
import (
	...
	router_commands "github.com/xtls/xray-core/app/router/command"
...
type observer struct {
	...
	dynSrcRuleIP         map[string]routing.DynamicRuleIP
	...
	obDestStat   map[obDestMetaKey]*obDestMeta

	obStatGCTicker *ospf.TickerFunc

	obConnTrackStatRw sync.RWMutex
	obConnTrackStat   map[netip.Addr]*obConnTrackMeta
}
...
func (s *dnsCircuit) observeDNSOutBound(ctx context.Context, domain string, result []net.IP, err error) {
	...
	if in == nil {
		xrayerrors.LogWarning(ctx, fmt.Sprintf("unexpected nil inbound from context. domain: %s", domain))
		return
	}
...
func (s *observer) updateConnTrackByDNS(sIn *session.Inbound, domain string, result []net.IP) {
	...
	default:
		xrayerrors.LogWarning(context.Background(), fmt.Sprintf("dropped observed DNS event from %s domain: %s", sIn.Source.String(), domain))
	}
}
...
func (s *observer) doObStatUpdate(ipNets []net.IPNet, e observedEvent) (canSendUpdates []net.IPNet, metas []*obDestMeta) {
	...
	if isUpdate {
		if isUpdatedAndSent {
			xrayerrors.LogDebug(context.Background(), fmt.Sprintf("updating domain: %s route CIDR: %s", e.d, PrettyPrintIPNet(ipNets...)))
		}
	} else {
...
func (s *observer) procEvent(e observedEvent) {
	...
	if err != nil {
		if errors.Is(err, common.ErrNoClue) {
			...
			xrayerrors.LogWarning(context.Background(), fmt.Sprintf("default outbound:%s for domain: %s ip: %v", finalTag, e.d, qualifiedIP))
		} else {
			xrayerrors.LogWarning(context.Background(), fmt.Sprintf("err pick route for domain: %s ip: %v: %v", e.d, qualifiedIP, err))
			return
		}
	} else {
...
func (s *observer) addConnTrack(e observedEvent, dests []net.IPNet, metas []*obDestMeta,
	...
	xrayerrors.LogDebug(context.Background(), fmt.Sprintf("adding default dynamic dest ip rule in %s: %s",
	...
	if ok {
		...
		xrayerrors.LogDebug(context.Background(), fmt.Sprintf("adding dynamic src %s conn-track rules in %s",
...
func (s *observer) removeDefaultDestRule(dest []net.IPNet) {
	xrayerrors.LogDebug(context.Background(), fmt.Sprintf("revoking default dynamic dest ip rule in %s: %s",
